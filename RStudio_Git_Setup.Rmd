...
### Chunk 2 — Build (replace your second chunk)

```r
```{r build_panel, message=TRUE}
DATA_DIR  <- here("data_raw")
XWALK_DIR <- here("xwalk")
OUT_PATH  <- here("output", "psid_family_year_2001_latest.parquet")

# Work just 2001/2003 for MVP
waves <- c(2001, 2003)
...
parse_sps_positions <- function(dict_path) {
  ...
  body <- str_replace_all(body, "\\t", " ")
  body <- str_squish(body)
  specs <- str_extract_all(body, "[A-Za-z0-9_]+\\s+\\d+\\s*-\\s*\\d+(?:\\s*\\([A-Za-z]\\))?") |> unlist()
  specs <- specs[!is.na(specs) & specs != ""]
  tibble(raw = specs) |>
    mutate(
      var   = str_match(raw, "^([A-Za-z0-9_]+)\\s+")[,2],
...
parse_do_positions <- function(dict_path) {
  ...
  tokens <- str_extract_all(lines, "(str\\d+|byte|int|long|double)?\\s*([A-Za-z0-9_]+)\\s+(\\d+)\\s*-\\s*(\\d+)") |> unlist()
  if (!length(tokens)) return(tibble(var=character(), start=integer(), end=integer(), width=integer(), type=character()))
  mat  <- str_match_all(tokens, "(str\\d+|byte|int|long|double)?\\s*([A-Za-z0-9_]+)\\s+(\\d+)\\s*-\\s*(\\d+)")
  out  <- purrr::map_dfr(mat, ~tibble(
    type_token = .x[,2], var = .x[,3], start = as.integer(.x[,4]), end = as.integer(.x[,5])
  ))
  out |>
    mutate(
      width = end - start + 1L,
      type  = if_else(str_detect(type_token, "^str\\d+$"), "character", "numeric")
    ) |>
    select(var, start, end, width, type) |>
    distinct(var, .keep_all = TRUE) |>
    arrange(start)
...
read_fixed_width <- function(stem) {
  ...
  fwf <- fwf_positions(dict$start, dict$end, dict$var)
  col_types <- paste(ifelse(dict$type == "character", "c", "d"), collapse = "")
  df <- read_fwf(file = txt, col_positions = fwf, col_types = col_types, na = c("", " ", ".", "NA"))
...
safe_read_xwalk <- function(fname) {
  ...
  read_csv(p, show_col_types = FALSE) |> mutate(wave = as.integer(wave))
}
...
state_xw   <- safe_read_xwalk("state_crosswalk.csv")
indocc_xw  <- safe_read_xwalk("industry_occupation_xwalk.csv")
roster_xw  <- safe_read_xwalk("roster_crosswalk.csv")  # optional for children<18

apply_xwalk <- function(df, xw, yr) {
  if (!nrow(xw)) return(tibble())
  xwy <- dplyr::filter(xw, wave == yr)
  if (!nrow(xwy)) return(tibble())
  missing <- setdiff(xwy$source, names(df))
  if (length(missing)) warning(glue("Wave {yr}: missing fields: {paste(missing, collapse=', ')}"))
  keep <- intersect(xwy$source, names(df))
  out  <- dplyr::select(df, all_of(keep))
...
load_indiv_wave <- function(yr) {
  ...
  if (!length(pick)) {
    warning(glue("No individual/roster file found for {yr}; children<18 will be NA"))
    return(NULL)
  }
...
get_roster_map <- function(yr) {
  ...
  list(
    age = rxy |> filter(varname == "age") |> pull(source) |> dplyr::first(),
    in_fu = rxy |> filter(varname == "in_fu") |> pull(source) |> dplyr::first(),
    rel = rxy |> filter(varname == "rel_to_head") |> pull(source) |> dplyr::first()
  )
}
...
build_wave <- function(yr) {
  ...
  # ---- Children < 18 in FU (optional; requires roster_crosswalk.csv + IND/PR)
  if (is.null(ind) || !nrow(roster_xw) || !any(roster_xw$wave == yr)) {
    children <- tibble(family_id = fam[[FAMILY_ID_COL]], n_children_u18 = NA_integer_)
  } else {
    rm <- get_roster_map(yr)
    if (any(is.na(unlist(rm))) || !all(unlist(rm) %in% names(ind))) {
      warning(glue("Wave {yr}: roster crosswalk incomplete; children<18 set to NA"))
      children <- tibble(family_id = fam[[FAMILY_ID_COL]], n_children_u18 = NA_integer_)
    } else {
      df_ind <- ind |>
        transmute(
      ...
      # Adjust these if your REL is numeric; leaving permissive for MVP
      in_fu_ok <- if (is.numeric(df_ind$in_fu)) df_ind$in_fu == 1 else df_ind$in_fu %in% c("1","Y","YES")
      rel_ok   <- rep(TRUE, nrow(df_ind))

      children <- df_ind |>
        filter(in_fu_ok, rel_ok, !is.na(age), age < 18) |>
        count(family_id, name = "n_children_u18")
    }
  }
...
# ----------------------------
# 7) Run across available waves and write
# ----------------------------
available_fam_years <- dir_ls(DATA_DIR, glob = "FAM*ER.txt") |> path_file() |> str_extract("\\d{4}") |> as.integer()
