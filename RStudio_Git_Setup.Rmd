...
find_family_id_col <- function(fam_df, yr) {
  ...
  # Heuristic fallback (kept, but warn loudly)
  cands <- grep("^ER\\d{4,6}02$", names(fam_df), value = TRUE)
  if (length(cands)) {
    warning("Wave ", yr, ": family_id not found in crosswalk; using heuristic among: ", paste(cands, collapse = ", "))
  ...
  stop("Could not identify family ID column for wave ", yr,
       ". Add to family_vars_crosswalk.csv: ", yr, ",family_id,ERxxxxx")
}

find_interview_year <- function(fam_df, yr) {
  # With your updated crosswalk, this should always hit.
  ...
  warning("Wave ", yr, ": interview_year source not found; falling back to wave year. Fix crosswalk.")
...
build_wave <- function(yr) {
  ...
  if (!is.null(ind) && nrow(roster_xw) && any(roster_xw$wave == yr)) {
    ...
    ind_fid <- rxy |> filter(varname=="family_id") |> pull(source) |> first()   # <-- ADD THIS TO roster_crosswalk.csv

    if (!any(is.na(c(age, in_fu, rel, ind_fid))) && all(c(age, in_fu, rel, ind_fid) %in% names(ind))) {
      kids <- ind |>
        transmute(
  ...
  # Hard check: one row per family per wave
...
# Final hard check across all waves
dups_panel <- panel %>% count(family_id, wave, name = "n") %>% filter(n > 1)
if (nrow(dups_panel)) {
